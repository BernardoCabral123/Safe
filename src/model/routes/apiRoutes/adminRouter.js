...
router.put(`/formandos`,(req,res)=>{
            
    dbConnection.query("UPDATE conta SET nome = ?, email= ? WHERE (`idConta` = ?)",
    [req.body.nome,req.body.email,req.body.idConta],
    (err,result)=>{
        if(err){
            console.log(err);
                res.status(422).send("Erro na atualização da turma");
...
router.post(`/formandos`,(req,res)=>{
    ...
    (err,result)=>{
        if(err){
            ...
            ['formando', req.body.email, bcryptjs.hashSync(escape(generatedPass,bcryptjs.genSaltSync(2))), req.body.nome],
            (err,resultt)=>{
                if(err){
                    console.log(err);
